---
title: "메모리 구조(Memory Model), 그리고 생애 첫 어셈블리어(Assembly Language)"
...
나도 메모리 구조에 대한 조금 더 구체적이고 명시적인 설명이 필요했기 때문에 바로 열었다.  
...
## 메모리 구조(Memory Model) 찾아왔다가 어셈블리어(Assembly Language) 만난 썰
